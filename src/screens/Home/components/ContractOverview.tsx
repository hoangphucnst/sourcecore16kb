...
// Component thẻ
const ContractCard: React.FC<ContractItem> = ({
  icon,
  count,
  label,
  bgColor,
...
interface ContractOverviewProps {
  numOfMortgageContract: number
  numOfCreditContract: number
}

// Component chính
const ContractOverview: React.FC<ContractOverviewProps> = ({
  numOfMortgageContract = 0,
  numOfCreditContract = 0,
}) => {
  const {THEME} = useAppStyles()
  const styles = useCallback(styleWithTheme, [THEME])(THEME)
  ...
  return (
    <View style={styles.container}>
      <AppText style={styles.title}>Hợp đồng</AppText>
