...
export type ScrollPickerProps<ItemT extends string | number> = {
  ...
  styleHighlight?: StyleProp<ViewStyle>
  headerScroll?: React.ReactNode | null
}
...
const ScrollPicker: {
  ...
  const highlightStyle: StyleProp<ViewStyle> = props.styleHighlight
    ? [
        {
          position: 'absolute',
        ...
        props.styleHighlight,
      ]
    : {
        position: 'absolute',
        top: (wrapperHeight - itemHeight) / 2,
        height: itemHeight,
        width: highlightWidth != deviceWidth ? highlightWidth : '100%',
  ...
  return (
    <View
      style={[
        wrapperStyle,
        {
          height: (Number(wrapperStyle?.height) || 0) + 35,
        },
      ]}>
      <View style={{height: 35}}>{props.headerScroll}</View>
